...
$(document).ready(function(){
    //inicia com a mascara para cpf com mais um número
    if($('#cpf').val().length > 14 && !$.browser.mobile){
        $('#cpf').setMask("99.999.999/9999-99");
    }else if(!$.browser.mobile){
        $('#cpf').setMask("999.999.999-999");
    }
 
    // se tiver mais do que 14 caracteres o campo de login ele irá mudar a mascara para cnpj
    $('body').on('keyup','#cpf', function(event) {
        var cpf = $('#cpf').unmaskedVal();
        if(cpf.length > 14 && !$.browser.mobile){
            $('#cpf').setMask("99.999.999/9999-99");
        }else if(cpf.length < 14 && !$.browser.mobile){
            $('#cpf').setMask("999.999.999-999");
        }
    });
    
    $('body').on('click', '[data-alert] a.close', function (e) {
        e.preventDefault();
