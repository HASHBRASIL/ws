    ...
    try {
        ...
        if ( UUID::is_uuid($nome)){
           $rsPessoa = $tpInformacao->getTpInformacaoByPerfisByPessoaByGrupo($SERVICO['metadata']['ws_perfil'], $nome, $grupo,true);
            ...
            if (count($rsPessoa) > 0) {
	            ...
	            $flashMsg->error('j� existem dados salvos para esta pessoa/entidade como '.$SERVICO['ws_target'].' em seu grupo.',true);
        ...
        foreach ($campos as $campo) {
            if($campo['tipo']!='Master') {
                ...
                //        parseJson(true, 'Campo obrigat�rio n�o preenchido ' . $campo['nome']);
                //    }
                //}
            }
        }
