$(document).ready(function() {
    ...
    $('.formaction').bind('click', function(e){
        ...
        if(show != 'reload') {
            ...
            if ($(form).children('input[type="file"]')) {
               $(form).attr('action', 'home.php?servico=' + servico);
               $(form).ajaxForm({ success: function(response){ trataRetornoAjax(show,response) } }).submit();
            } else {
...
function trataRetornoAjax(show,response){
    switch(show){
        case 'paginacao':
...
function ajaxFunction(servico, data, show){
    $.ajax({
        ...
        success: function(response, status, XMLHttpRequest) {
            ...
            trataRetornoAjax(show,response);
        },
