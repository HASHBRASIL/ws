    ...
    $('#novoForm').click(function(e){
        ...
        $('#dialog-form').dialog({
                ...
                close: function(){
                    ...
                    $('#register_public').prop('checked', false);
                    $('#register_public').uniform();
...
function ajaxSalvar($dialog, type){
    ...
    var data = {nome        : nome,
                ...
                register_public : $('#register_public').is(':checked') ? $('#register_public').val(): null
                };
    $.ajax({
        type: "POST",
        url: url,
...
function addTree(data, type){
    var branches = null;
    if( type == "grupo"){
        branches = $("<li class='class id_grupo_"+data.id+"'><span class='grupo_"+data.id+"'>"+data.nome+"</span>"+
              "<div class='action' > "+
              " <a id_grupo='"+data.id+"' href='/material/grupo/form' class='edit'>"+
                  "<img src='/images/tab-edit-icon.png' data-tooltip title='editar Grupo'/>"+
              "</a>"+
              " <a id='"+data.id+"' href='/material/sub-grupo/form' class='add-subClass'>"+
              '<img src="/images/tab-add-icon.png" data-tooltip title="adicionar subClasse"/> '+
              '</a> <a id="'+data.id+'" href="/material/item/grid/id_grupo/'+data.id+'" class="add add_action_grupo_'+data.id+'">'+
              '<img src="/images/package_add.png" data-tooltip style="height: 13px;" title="Produto"/>'+
          ...
          if($(".id_grupo_"+id_grupo+" > ul").length > 0){
              branches = $("<li class='class id_subgrupo_"+data.id+"'><span class='subgrupo_"+data.id+"'>"+data.nome+"</span>"+
                      "<div class='subAction' >"+
                      " <a id_subgrupo='"+data.id+"' href='/material/sub-grupo/form' id_grupo='"+id_grupo+"' class='edit'>"+
                          "<img src='/images/tab-edit-icon.png' data-tooltip title='editar Sub-Grupo' />"+
                      "</a>"+
                      " <a id='"+data.id+"' href='/material/classe/form' class='add-class'>"+
                      '<img src="/images/tab-add-icon.png" data-tooltip title="adicionar Classe"/> '+
                      '</a> <a id="'+data.id+'" href="/material/item/grid/id_subgrupo/'+data.id+'" class="add add_action_subgrupo_'+data.id+'">'+
                      '<img src="/images/package_add.png" data-tooltip style=" height: 13px;" title="Produto"/>'+
        ...
        if($(".id_subgrupo_"+id_subgrupo+" > ul").length > 0){
            branches = $("<li class='class id_classe_"+data.id+"'><span class='classe_"+data.id+"'>"+data.nome+"</span>"+
                    "<div class='classe' >"+
                    " <a id_classe='"+data.id+"' href='/material/classe/form' id_subgrupo='"+id_subgrupo+"' class='edit'>"+
                        "<img src='/images/tab-edit-icon.png' data-tooltip title='editar Classe' />"+
                    "</a>"+
                    ' <a id="'+data.id+'" href="/material/item/grid/id_classe/'+data.id+'" class="add">'+
                    '<img src="/images/package_add.png" data-tooltip title="Item" style="height: 13px;" />'+
            ...
            branches = $("<ul><li class='class id_classe_"+data.id+"'><span class='classe_"+data.id+"'>"+data.nome+"</span>"+
                    ...
                    "</a>"+
                    ' <a id="'+data.id+'" href="/material/item/grid/id_classe/'+data.id+'" class="add">'+
                    '<img src="/images/package_add.png" data-tooltip title="Produto" style="height: 13px;" />'+
