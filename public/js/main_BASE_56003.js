        ...
        if (typeof trataForm == "function") {
            var data = trataForm(form);
             ...
             if ($(form).attr('enctype') == 'multipart/form-data') {
                ...
                data = $(form).find(':input').not('select.select2-hidden-accessible').serializeArray();
            }
//            var data = $(form).find(':input').not('select.select2-hidden-accessible').serializeArray();
        }
    ...
    $('body').on("click", ".listaction", function(e){
        e.preventDefault();
        var servico = $(this).attr('data-servico');
        var show    = $(this).attr('data-show');
    ...
    $.each(data, function(i, row) {
        ...
        $.each(clone.find('td'), function(k,v) {
            if ($(v).data('checkbox') == 1) {
                $(v).find('label').attr('for', 'checkbox-' + row.id);
                $(v).find('input').attr('id', 'checkbox-' + row.id);
